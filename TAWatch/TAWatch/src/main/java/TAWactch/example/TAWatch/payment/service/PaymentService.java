...
import TAWactch.example.TAWatch.payment.entity.PaymentTransaction;
import TAWactch.example.TAWatch.payment.repository.PaymentTransactionRepo;
...
public class PaymentService {
    ...
    public VnpayInitiateResponse initiateVnpay(VnpayPaymentRequest request) {
        ...
        String encodedHashString = VNPayUtil.buildQueryString(vnpParams);
        String secureHashRaw = VNPayUtil.hmacSHA512(vnpayHashSecret, rawHashString);
        String secureHashEncoded = VNPayUtil.hmacSHA512(vnpayHashSecret, encodedHashString);
        String urlParams = encodedHashString;


        // VNPay spec (PHP + Java official samples): hash trên giá trị đã URL-encode
        String secureHash = secureHashEncoded;
    ...
    public PaymentTransactionResponse handleVnpayCallback(Map<String, String> params) {
        ...
        if (success && order.getOrderStatus() == OrderStatusType.PENDING) {
            order.setOrderStatus(OrderStatusType.CONFIRMED);
        }
