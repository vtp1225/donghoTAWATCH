...
public class JwtUtil {
    ...
    public boolean isTokenValid(String token) {
        try {
            SignedJWT jwt = SignedJWT.parse(token);
            boolean isValidSignature = jwt.verify(new MACVerifier(secretBytes()));
            ...
            boolean isNotResetToken = !"RESET_PASSWORD".equals(type);
            
            return isValidSignature && isNotExpired && isNotResetToken;
        } catch (Exception e) {
            return false;
        }
    }
