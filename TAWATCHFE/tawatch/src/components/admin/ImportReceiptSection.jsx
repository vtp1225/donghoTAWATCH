...
function DetailModal({ receipt, onClose, onConfirm, onCancel, confirming, cancelling }) {
  console.log('DetailModal receipt:', receipt) // Debugging line
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <div className="relative bg-surface-container-low border border-outline-variant/20 p-8 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-start mb-6">
          ...
          <span className={`px-2 py-0.5 text-[9px] font-label-caps tracking-wider border ${s.color}`}>{s.label}</span>
        </div>
        ...
        {receipt.status === 'DRAFT' && (
          <div className="flex gap-3">
            <button onClick={onCancel} disabled={cancelling} className="px-6 py-2 border border-outline-variant/30 text-on-surface-variant font-label-caps text-xs tracking-widest hover:border-error hover:text-error transition-all disabled:opacity-50">
              {cancelling ? 'ĐANG HUỶ...' : 'HUỶ PHIẾU'}
            </button>
            <button onClick={onConfirm} disabled={confirming} className="px-8 py-2 bg-primary text-background font-label-caps text-xs tracking-widest hover:bg-primary/80 transition-all disabled:opacity-50">
...
function CreateModal({ open, suppliers, currentUserId, onClose, onSuccess }) {
  ...
  useEffect(() => {
    if (open) {
      ...
      setSupplierId(''); setNote(''); setError('')
  ...
  function updateItem(idx, field, value) {
    setItems((prev) => {
      ...
      if (field === 'watchId') { next[idx].watchVariantId = ''; loadVariants(value) }
      return next
    })
  }

  function calcTotal() {
    return items.reduce((sum, it) => sum + (parseFloat(it.unitCost) || 0) * (parseInt(it.quantity) || 0), 0)
  }
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <form onSubmit={handleSubmit} className="relative bg-surface-container-low border border-outline-variant/20 p-8 max-w-3xl w-full mx-4 max-h-[92vh] overflow-y-auto">
        <p className="font-label-caps text-[9px] tracking-[0.3em] text-primary mb-1 uppercase">Nhập kho</p>
        <h3 className="font-headline-sm text-headline-sm text-on-background mb-6">Tạo phiếu nhập mới</h3>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div>
            ...
            <select value={supplierId} onChange={(e) => setSupplierId(e.target.value)} className="w-full border-b border-outline-variant/25 bg-transparent py-2 text-sm outline-none focus:border-primary text-on-background">
          ...
          <div>
            ...
            <input type="date" value={importDate} onChange={(e) => setImportDate(e.target.value)} className="w-full border-b border-outline-variant/25 bg-transparent py-2 text-sm outline-none focus:border-primary text-on-background" />
          </div>
          <div className="col-span-2">
            <label className="mb-1 block font-label-caps text-[9px] tracking-[0.25em] text-on-surface-variant/60 uppercase">Ghi chú</label>
            <input value={note} onChange={(e) => setNote(e.target.value)} className="w-full border-b border-outline-variant/25 bg-transparent py-2 text-sm outline-none focus:border-primary text-on-background" placeholder="Ghi chú phiếu nhập..." />
          </div>
        </div>
        ...
        <div className="mb-4">
          <div className="flex justify-between items-center mb-3">
            ...
            <button type="button" onClick={() => setItems((p) => [...p, { watchVariantId: '', watchId: '', quantity: 1, unitCost: '' }])} className="flex items-center gap-1 text-xs text-primary hover:text-primary/70 transition-colors">
            ...
            {items.map((item, idx) => (
              <div key={idx} className="grid grid-cols-[2fr_2fr_1fr_1.5fr_auto] gap-3 items-end border-b border-outline-variant/10 pb-3">
                <div>
                  ...
                  <select value={item.watchId} onChange={(e) => updateItem(idx, 'watchId', e.target.value)} className="w-full border-b border-outline-variant/20 bg-transparent py-1.5 text-sm outline-none focus:border-primary text-on-background">
                ...
                <div>
                  ...
                  <select value={item.watchVariantId} onChange={(e) => updateItem(idx, 'watchVariantId', e.target.value)} disabled={!item.watchId} className="w-full border-b border-outline-variant/20 bg-transparent py-1.5 text-sm outline-none focus:border-primary text-on-background disabled:opacity-40">
                    <option value="">-- Chọn biến thể --</option>
                    {(variantsByWatch[item.watchId] || []).map((v) => (
                      <option key={v.id} value={v.id}>
                        {[v.dialColorName && `Mặt ${v.dialColorName}`, v.strapColorName && `Dây ${v.strapColorName}`, v.caseSizeMm && `${v.caseSizeMm}mm`].filter(Boolean).join(' / ') || 'Mặc định'} (Tồn: {v.stockQuantity})
                      </option>
                    ))}
                  </select>
                </div>
                <div>
                  ...
                  <input type="number" min="1" value={item.quantity} onChange={(e) => updateItem(idx, 'quantity', e.target.value)} className="w-full border-b border-outline-variant/20 bg-transparent py-1.5 text-sm outline-none focus:border-primary text-on-background" />
                </div>
                <div>
                  ...
                  <input type="number" min="0" value={item.unitCost} onChange={(e) => updateItem(idx, 'unitCost', e.target.value)} className="w-full border-b border-outline-variant/20 bg-transparent py-1.5 text-sm outline-none focus:border-primary text-on-background" placeholder="0" />
                </div>
                <button type="button" onClick={() => setItems((p) => p.filter((_, i) => i !== idx))} disabled={items.length === 1} className="p-1.5 text-error/40 hover:text-error disabled:opacity-20 transition-colors mb-1">
        ...
        <div className="flex justify-between items-center mt-6">
          ...
          <div className="flex gap-3">
            ...
            <button type="submit" disabled={saving} className="px-8 py-2 bg-primary text-background font-label-caps text-xs tracking-widest hover:bg-primary/80 transition-all disabled:opacity-50">{saving ? 'ĐANG TẠO...' : 'TẠO PHIẾU'}</button>
          </div>
        </div>
      </form>
    </div>
...
export default function ImportReceiptSection({ onCreateClick, createOpen, onCreateClose }) {
  const { user } = useAuth()
  ...
  async function handleConfirm() {
    ...
    setConfirming(true)
    try {
      await importReceiptService.confirm(viewReceipt.id)
      // Re-fetch để lấy trạng thái mới nhất từ server
      const refreshed = await importReceiptService.getById(viewReceipt.id)
      setViewReceipt(refreshed)
      // Cập nhật bảng ngay không chờ load()
      setReceipts((prev) =>
        prev.map((r) => r.id === refreshed.id ? { ...r, status: refreshed.status } : r)
      )
    } catch (err) {
  ...
  async function handleCancel() {
    ...
    setCancelling(true)
    try {
      await importReceiptService.cancel(viewReceipt.id)
      const refreshed = await importReceiptService.getById(viewReceipt.id)
      setViewReceipt(refreshed)
      setReceipts((prev) =>
        prev.map((r) => r.id === refreshed.id ? { ...r, status: refreshed.status } : r)
  ...
  return (
    <section>
      ...
      <div className="flex gap-2 mb-6">
        {['ALL', 'DRAFT', 'CONFIRMED', 'COMPLETED', 'CANCELLED', 'REJECTED'].map((s) => (
          <button key={s} onClick={() => setStatusFilter(s)} className={`px-4 py-1.5 font-label-caps text-[9px] tracking-widest border transition-all ${statusFilter === s ? 'border-primary text-primary bg-primary/10' : 'border-outline-variant/20 text-on-surface-variant/50 hover:border-primary/40'}`}>
            {s === 'ALL' ? 'TẤT CẢ' : STATUS_LABEL[s]?.label?.toUpperCase() || s}
          </button>
        ))}
        <button onClick={load} className="ml-auto material-symbols-outlined text-on-surface-variant hover:text-primary transition-colors text-xl" title="Làm mới">refresh</button>
      </div>
      ...
      <div className="border border-outline-variant/15 bg-surface-container-low/30">
        <div className="grid grid-cols-[1.5fr_1.5fr_1fr_1.5fr_1fr_1fr] gap-4 px-6 py-3 border-b border-outline-variant/10">
          {['Mã phiếu', 'Nhà cung cấp', 'Ngày nhập', 'Tổng tiền', 'Trạng thái', 'Người tạo'].map((h) => (
          ...
          filtered.map((r, idx) => {
            ...
            return (
              <div
                ...
                className={`grid grid-cols-[1.5fr_1.5fr_1fr_1.5fr_1fr_1fr] gap-4 items-center px-6 py-4 cursor-pointer hover:bg-surface-container/50 transition-colors ${idx !== filtered.length - 1 ? 'border-b border-outline-variant/10' : ''}`}
              >
                ...
                <span className={`inline-block px-2 py-0.5 text-[9px] font-label-caps tracking-wider border ${s.color}`}>{s.label}</span>
